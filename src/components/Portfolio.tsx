...
const Project = ({title , skills , description , link , github , impo , preview} : projectType) => {
    ...
    return(
    <div onClick={handleClick} className={`${isOpened ? 'bg-white bg-opacity-70' : ''} lg:w-[45%] rounded overflow-hidden shadow-lg my-6 hover:cursor-pointer md:hover:shadow-orange-300 transition-all hue-rotate-60 mx-2 h-fit`}>
        ...
        <div className="px-2 sm:px-6 py-2 sm:py-4 overflow-hidden">
            ...
            <p className={`font-monserrat text-[0.4rem]/[1rem] lg:text-xs sm:leading-8 ${isOpened ? '' : 'truncate' } transition-all`}>{description}</p>
        </div>
