...
const Project = ({title , skills , description , link , github , impo , preview} : projectType) => {
    ...
    <div onClick={handleClick} className={`${isOpened ? 'bg-black bg-opacity-70' : ''} w-full rounded overflow-hidden shadow-lg my-6 hover:cursor-pointer md:hover:shadow-orange-300 transition-all hue-rotate-60`}>
        ...
        <div className="px-2 sm:px-6 py-4 overflow-hidden">
            ...
            <p className={`font-monserrat text-xs leading-6 ${isOpened ? '' : 'truncate' } transition-all`}>{description}</p>
        </div>
        ...
        <div className="px-6 py-2">
            {skills.map(skill =>{
                return  <span key={skill} className="inline-block bg-orange-100 rounded-full px-2 py-1 text-xs font-semibold text-gray-700 mr-1 sm:mr-2 mb-2 hover:bg-yellow-500 hover:cursor-default hover:text-gray-100 transition-all">{skill}</span>
                })}
        </div>
    </div>
    )
